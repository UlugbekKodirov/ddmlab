...
def slicing_ngram_generator(seq, k):
    ...
    seq_secondpart = seq.split(" ")[1]
    for i in range(len(seq_secondpart)-k+1):
        yield seq_secondpart[i:i+k]
...
kmers = sc.textFile("hdfs:///user/hadoop/transmembrane/{}".format(INPUT_FILE)).flatMap(lambda seq: set(USED_FUNC(seq, KMERS_N)))
kmers = kmers.map(lambda kmer: (kmer, 1)).reduceByKey(add)
kmers.saveAsTextFile("hdfs:///user/hadoop/transmembrane_output/{}".format(OUTPUT_DIR))
