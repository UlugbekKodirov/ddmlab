...
if __name__ == "__main__":
    ...
    sdf = spark.read.csv('./transmembrane/vecProteinLabel_ratio_1to1_symmetricdifference_top500positive_top500negative.csv', header=True, inferSchema=True, enforceSchema=False)
    datas = sdf.rdd.map(lambda x: LabeledPoint(x[0], x[1:]))
    # Split the data into training and test sets (30% held out for testing)
    (trainingData, testData) = datas.randomSplit([0.7, 0.3])
    # train a DecisionTree model.

    for maxDepth in range(15, 30):
        #maxDepth = 5
        maxBins = 400
        impurity = 'gini'
    ...
    # evaluation of the model
        output_name = "decision_tree" + str(maxDepth) + "_" + str(maxBins) + "_" + impurity + '_ratio_1_1_symmetricdifference_top500frombothclasses'
